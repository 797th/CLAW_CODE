#!/usr/bin/env bash
# Claw Code installer — macOS backend.
#
# Invoked by install/install.py. Also runnable standalone:
#   bash install/backends/macos.sh --release
#
# Builds both `claw` and `cliclaw` from rust/, copies them into a bin directory,
...
while [ "$#" -gt 0 ]; do
    case "$1" in
        ...
        --install-dir)
            ...
            INSTALL_DIR="$2"; shift;;
        -h|--help)
            echo "Usage: bash install/backends/macos.sh [--release|--debug] [--no-verify] [--install-dir DIR] [--no-path-update]"
            exit 0;;
...
# ---------------------------------------------------------------------------
# PATH update (idempotent) — zsh + bash
# ---------------------------------------------------------------------------
